...
**Markout** is an autonomous Uniswap v4 hook with **two lanes on one 24 s clock**. Spot fills immediately at 3 bps and escrows a **live-quoted** reversion-insurance premium (starts at 20 bps, donate +3 / refund −1, clamp 5–60), settled with a **normalized mean-reversion oracle** that lives entirely inside the hook. Batch is opt-in: opposing orders net at the epoch TWAP without touching the AMM; unmatched size is one bonded residual spot swap.

Over each **spot** trade's **immutable** settlement window — `[bondTime, bondTime + 24 s]`, two 12 s blocks:

* **Reversion (Benign):** If at least **50% of the trade's own price impact** (measured in tick space, time-weighted) reverted back toward the pre-swap price, the trade was uninformed. The premium is **refunded — delivered to the trader in the settlement transaction itself**. A separate claim exists only when token delivery failed and remains retryable.
* **Sustain (Toxic):** If the time-weighted price sustained (residual impact above the 50% frontier, or moved further away), the trade was informed price discovery. The premium is **donated to in-range LPs** as a socialized dividend, credited inside `settle` when L > 0 (deferred to `flushDonation` only at zero liquidity).
...
**Honest scope:** the oracle does not catch slow trend flow (no reversion within the window), the front leg of an atomic **spot** sandwich (the backrun's reversion refunds it — the backrun leg itself donates), a **live batch residual** (unmatched size is an unbounded-limit spot swap; cancelled orders do not move the TWAP; exact two-sided nets never hit the curve), and donations go to whoever is in range at credit time, not the specific LPs who carried the toxic inventory. Dust donates can walk the live quote toward 60 bps; refunds walk it back 1 bp at a time. The premium is a tax, not an LVR hedge.

## 3. Settlement Architecture

### Premium Payable Through Any Router

The live premium is charged via v4's hook-delta mechanism: for exact-in swaps the hook returns it on the *specified* delta in `beforeSwap`; for exact-out swaps on the *unspecified* delta in `afterSwap`. It lands in the **swap caller's own PoolManager delta**, so any router that can settle a normal v4 swap can pay it — Universal Router, `PoolSwapTest`, or an integrator's own contract. The provided `MarkoutRouter` is convenience (deadline, exact-in minimum output, exact-out maximum input including the live premium, strict ERC-20 handling, native support, beneficiary declaration in `hookData`), never a gate. `MarkoutRouter.BOND_BPS` is the genesis default only — the hook quotes `premiumBps`.
...
* **Exact premium, both directions:** the charge is `premiumBps` of the realized input taken from the post-swap `balanceDelta`; `slot0` is never used for sizing. Exact-in charges are reconciled to realized fills (partial-fill overcharge is returned to the trader in the same swap). `premiumQuoteFor` matches the charge.
* **Immediate fill:** swaps execute on the AMM curve in the origin transaction.
* **No dust bypasses:** premiums that round to zero revert `SwapTooSmall`.
...
2. **As an organic trader**, I want my uninformed swap verified by natural arbitrageurs reverting the price behind me, with my live premium **paid back in the settlement transaction**.
3. **As a router integrator**, I want to pay the premium with nothing beyond normal v4 settlement — no Markout allowlist, no special call — so my existing router works unchanged.
...
2. **MarkoutHook (BaseHook):** delta-charged live premium, previous-tick accumulator with append-only history, permissionless terminal settlement (at-settle refund payment, retryable claim), credit-at-settle LP donations, batch epochs (`placeBatchOrder` / `cancelBatchOrder` / `clearBatch`), residual via immutable `MarkoutBatchRouter` child, strict escrow accounting.
3. **MarkoutRouter (Convenience Integrator):** deadline, exact-in min-out / exact-out max-in (live premium included), strict ERC-20 handling, native support, beneficiary declaration. Not a gate.
...
* **Invariant fuzz (handler-based, 256 runs × 500 calls):** five invariants — escrow coverage, liability identity, verdict immutability, bounded per-trade release, batch custody covered.
