...
## Purpose
Markout's face: warm-paper light theme — cream canvas #faf8f2, ink text, magenta brand #B5276F, gold accent — Fraunces display + Hanken Grotesk UI + JetBrains Mono for addresses/amounts, Tailwind + shadcn/ui new-york primitives, sticky `SiteNav` with mobile Sheet drawer, hatch strip + marquee facts ticker, `.tape` / `.panel` / `.eyebrow` / `.stat-row` / `.addr` vocabulary, `max-w-content` editorial rhythm, and a full-bleed ink differentiator band. Three routes: `/` landing (hero with the **live `LandingTape`** — the real MemoryTape streaming slot0 with no wallet, no KPI strip in the hero — how-it-works cards, vanilla-vs-Markout LP ledgers with the dividend row, ink band, live-on-testnet address grid + hosted-URL line + video hole, final CTA with the Chrome+MetaMask+Sepolia judge note), `/app` memory console (Pipeline Connect→Mint→LP-or-skip→Swap-spot-or-batch→Settle; `SwapPanel` + `BatchPanel` + `LpPanel` official PosM; paper MemoryTape; **`LpSeat`** pool-wide liquidity/pending/flush), `/docs` (TOC, Formula/Callout/Step chrome, honest-limits incl. spot sandwich, tax-not-LVR, credit-at-settle, residual-is-spot). All wallet/tx wiring lives in `lib/markout.tsx` (MarkoutProvider) so pages stay in sync. Hosted at https://markout-nine.vercel.app (also in README, footer, docs, `layout.tsx` metadataBase).
## What This Controls
...
Working against the **2026-09-02** deployment (hook `0x1e9A03…`, batch router `0xC9aaB8…`, router `0xF06737…`, TOKEN0=MDB `0x41a9c2…`, TOKEN1=MDA `0xae0FE2…`, pool `0xa6a2c6…`). Honesty cut this session: landing two-lane copy + 5 honest-limit cards + BATCH_ROUTER + 09-02 `PROOFS`; `/app` token labels match pool order (zeroForOne sells MDB); `demoDonate` approves the router and skips `flushDonation` when pending is already 0 (credit-at-settle); `lpTokenId` cleared on full remove; `pilot` union includes `"batch"`; GuideBanner stays through the swap step; scripts `live-check`/`history-check` pointed at 09-02 + `premiumQuoteFor` + env→tenderly→publicnode. Do not claim sandwiches empty on the residual path. `src/` === live bytecode — no frontend-driven redeploy.
