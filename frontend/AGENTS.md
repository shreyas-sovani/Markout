...
### 2026-08-27 — Voltaire clone scrapped mid-flight; Lambda clone shipped
- **Change**: the hydrographic dark redesign (and a partially-read Voltaire clone that never landed) were replaced wholesale by a clone of `lambda-protocol/frontend/`: tailwind.config.ts / postcss / components.json / globals.css / lib/utils.ts / ui primitives (button, card, badge, separator, sheet) copied verbatim; Brand (magenta "M" seal), SiteNav (copied), SiteFooter (Markout columns), Connect + NetworkBanner rebuilt on `lib/wallet.ts` (no wagmi), Pipeline (Connect→Mint→Swap→Settle), HeroVisual (paper collage: bonded swap / memory tape / verdict route), MemoryTape (the 24 s tape restyled onto paper: ink dashed pre, ink post, gold frontier, magenta trace + window shade + sweep, verdict captions). The god-page was split into `lib/markout.tsx` (MarkoutProvider: all polls, recovery, previews, actions, demos) + three routes. Facts ticker, `.tape` headers, ledger table with outcome badges, and docs Formula/Callout/Step chrome all follow Lambda's vocabulary.
- **Reasoning**: directive — Lambda is the design source of truth; copy the system, not the product; do not invent an identity, do not dark-mode, do not use Voltaire's forest sidebar. Provider split keeps pages in sync without regressing any tx-correctness rule (exact approvals, simulate-first, receipt-parsed tradeId, chunked getLogs, chain-time countdown).
- **Rejected alternative(s)**: continuing the Voltaire clone (superseded by directive); dark hydrographic look (banned); wagmi for the nav Connect (banned — must use `lib/wallet.ts`); Lambda's product copy (Hyperliquid/Reactive/h=0.65 — replaced with Markout facts throughout).
- **Task/session**: Lambda-clone directive, 2026-08-27.
