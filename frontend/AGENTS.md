...
One-page Next.js app (App Router, single client page) wired to the canonical-Sepolia deployment, styled as a **hydrographic chart-recorder**: the product's holdable artifact is "The 24-second memory" — a full-width live instrument recording the pool tick as a phosphor trace, with the active trade's fixed window, pre/post/frontier lines, window average, and verdict overlaid. Same wiring as before: injected-wallet connect, capped-faucet mint, exact approvals with pre-write simulation, slippage + deadline protected swaps, deterministic receipt parsing (never a global last-trade pointer), refresh-safe trade recovery, settle / claimRefund / flushDonation from panel and ledger, one-click deterministic Refund and Donate pilots, RPC fallback + health chip, network switching, aria-live regions, mobile-card ledger, plus a quiet traction strip (cumulative value flushed to LPs from `DonationFlushed` logs) and explicit human-owned holes for the demo video and hosted URL.
...
Working: `next build` passes (static, ~103 kB shared JS). Read pipeline (poolId, slot0 extsload, chunked logs, trades multicall, previewTrade, traction strip) and tx pipeline (mint → exact approve → simulated swap → settle → flush) validated against live canonical-Sepolia state on 2026-08-27 (`scripts/live-check.mjs`, `scripts/history-check.mjs`; SSR render check for hero/instrument/seats/traction/holes). Constants point at the 2026-08-27 deployment (hook `0x027C…`, 24 s window, outcome enum 1=Refunded/2=RefundPending/3=Donated, at-settle refunds). The 2026-08-25 addresses are stale.
