...
- **COVERAGE-COMPAT TWO-LANE CUT LIVE 2026-09-02, src === bytecode, no drift.** Hook `0x1e9A034b21aB19D00556b429C281f9B29d8BB0Cc`, immutable batch-router child `0xC9aaB8CaD29bE99A36653Ec5A6d78278C84D4067`, router `0xF06737dCbA252D276deCC0f6F0f2102AD20c7535`, tokens `0x41a9c2…`(MDB, c0)/`0xae0FE2…`(MDA, c1), pool `0xa6a2c6…` on the canonical Sepolia PM; all five Etherscan-verified; seeded 10e18 via the official PositionManager. Lanes: SPOT (instant 3 bps fill + live-quoted premium 5–60 bps stepped by settle history: donate +3 / refund −1) and BATCH (opt-in 24 s epochs on the same clock, explicit custody, permissionless uniform-TWAP clears via `clearBatch`, residual = one bonded swap through the child router because v4-core skips hook callbacks on self-calls). Donate verdicts credit in-range LPs INSIDE the settle tx when L > 0.
- Tests: 55/55 intended (engine 9 + integration 38 incl. batch/premium/credit-at-settle + fork 3 + fuzz 5 invariants). Engine/integration/fork verified green at `optimizer_runs = 20000`; **the fuzz suite was skipped this session at the human's request — run `forge test` fully before submitting.** foundry.toml now pins 20k runs (44M pushed the two-lane hook past EIP-170; 20k = 23,970 bytes and v4-core still compiles).
- Live proof pack (2026-09-02, our txs): buy 11616518 + 1.01× reverse next block (Δ12 s) → `Settled(outcome 1)` + `RefundClaimed` at settle (`0xe354716c…`); unreversed swap → `Settled(outcome 3)` + `DonationFlushed` in the SAME settle tx (`0xda709887…`); pre-signed buy+sell batch orders → `clearBatch` uniform TWAP, buy-side residual 0 (`0xdb3a18f6…`). `forge coverage --ir-minimum` now compiles (clearBatch stack-refactored into helpers).
- Frontend: two-lane `/app` (spot panel with live premium, `BatchPanel` with epoch countdown/place/cancel/clear + one-wallet netting demo, `LpPanel` official PosM+Permit2 add/remove, `LpSeat` pool-wide), landing live tape + 4-card honest-limits (sandwich / tax-not-LVR / credit-at-settle / lone-order-is-a-TWAP), docs batch section; hosted **https://markout-nine.vercel.app** (Vercel deploys `main`; this cut not yet pushed/deployed — human owns that).
- Operator EOA `0xFeAf…690A` is EIP-7702-delegated: broadcast with `--slow` works end-to-end now; proof txs use pre-signed pairs + explicit `--gas-limit 3000000` on settle/clear (inline LP credit is an external self-call a bare estimate can starve).
- Remaining human work: run full `forge test`, demo video (hole labeled), commit/push/Vercel deploy of this cut, final submission.
- Frontend CSS: never run `next build` and `next dev` against the same `frontend/.next`. Tailwind config is CJS `frontend/tailwind.config.js` + `postcss.config.mjs`.
