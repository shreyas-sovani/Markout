...
- **COVERAGE-COMPAT TWO-LANE CUT LIVE 2026-09-02, src === bytecode, no drift.** Hook `0x1e9A034b21aB19D00556b429C281f9B29d8BB0Cc`, immutable batch-router child `0xC9aaB8CaD29bE99A36653Ec5A6d78278C84D4067`, router `0xF06737dCbA252D276deCC0f6F0f2102AD20c7535`, tokens `0x41a9c2…`(MDB, c0)/`0xae0FE2…`(MDA, c1), pool `0xa6a2c6…` on the canonical Sepolia PM; all five Etherscan-verified; seeded 10e18 via the official PositionManager. Lanes: SPOT (instant 3 bps fill + live-quoted premium 5–60 bps, donate +3 / refund −1) and BATCH (opt-in 24 s epochs, explicit custody, `clearBatch` uniform TWAP, residual = one unbounded-limit bonded swap through the child — named honest limit, not "sandwiches empty"). Donate verdicts credit in-range LPs INSIDE the settle tx when L > 0. **Do not redeploy** unless bytecode must change; this honesty cut is docs/UI/tests only.
- Tests: engine 9 + integration 39 (incl. `test_batch_cancelledOrders_doNotMoveClearingPrice` + `test_batch_residualSpotSwap_inheritsSandwichHonestLimit`) + fork 3 + fuzz 5 invariants = **55**. foundry.toml `optimizer_runs = 20000`.
- Live proof pack (2026-09-02, our txs) in `frontend/lib/contracts.ts` `PROOFS`: refund settle `0xe354716c…`; donate+credit-in-settle `0xda709887…`; two-sided batch clear `0xdb3a18f6…`.
- Frontend honesty cut: two-lane landing (5 honest-limit cards incl. residual-is-spot), token labels match pool order, `demoDonate` approves + skips flush when credited at settle, `lpTokenId` cleared on remove, `pilot: batch`, GuideBanner through swap step, scripts on 09-02 + `premiumQuoteFor`. Hosted **https://markout-nine.vercel.app** (Vercel deploys `main`; this cut not yet pushed — human owns that).
- Operator EOA `0xFeAf…690A` is EIP-7702-delegated: `--slow` / sequential sends. Notional-weighted premium and residual price limits skipped (redeploy).
- Remaining human work: demo video (hole labeled), push/Vercel deploy, final submission.
